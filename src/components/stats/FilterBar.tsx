...
type Props = {
  ...
  showTopN?: boolean;
  /** show the compare-period toggle (default: true on overview, false on drill pages) */
  showCompare?: boolean;
};
...
export function FilterBar({
  ...
  const currentCompare = searchParams.get("compare") ?? "off";
  ...
  const hasActiveFilter =
    currentRange !== "current-month" ||
    selectedSlugs.length > 0 ||
    currentTopN !== "20" ||
    currentCompare !== "off" ||
    isCustomActive;

  const filteredAos = aoSearch
    ? aoOptions.filter((a) =>
        a.aoName.toLowerCase().includes(aoSearch.toLowerCase()),
      )
    : aoOptions;
  ...
  return (
    <ClipFrame padding="p-5" className="mb-6">
      <div className="flex items-center justify-between gap-3 mb-3">
        ...
        {hasActiveFilter && (
          <button
            type="button"
            onClick={resetAll}
            className="font-mono text-[10px] tracking-[.15em] uppercase text-muted hover:text-ink underline underline-offset-4"
          >
            reset all
          </button>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 font-mono text-xs">
        ...
        {showTopN && (
          <div className="md:col-span-2 flex items-center gap-2">
            <label htmlFor="topn-filter" className="text-muted">
              top:
            </label>
            <select
              id="topn-filter"
      ...
      {showCompare && (
        <div className="mt-4 pt-4 border-t border-black/10 flex flex-wrap items-center gap-2 font-mono text-xs">
          <span className="text-muted">compare:</span>
          {[
            { v: "off", label: "Off" },
            { v: "prior", label: "Prior period" },
            { v: "yoy", label: "Year over year" },
          ].map((opt) => {
            const on = currentCompare === opt.v;
            return (
              <button
                key={opt.v}
                type="button"
                onClick={() => setParam("compare", opt.v)}
                aria-pressed={on}
          ...
          <span className="ml-auto text-muted text-[10px] tracking-[.15em] uppercase">
            // KPI deltas + trend overlay
          </span>
        </div>
      )}
        ...
        <div className="mt-4 pt-4 border-t border-black/10">
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            ...
            {aoOptions.length > 8 && (
              <input
                ...
                className="ml-auto border border-black/20 px-2 py-1 font-mono text-[11px] w-44"
                aria-label="Search AOs"
              />
            )}
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {filteredAos.length === 0 && (
              <span className="font-mono text-[11px] text-muted px-2 py-1.5">
                // no AOs match
              </span>
            )}
          </div>
      ...
      {showCustom && (
        <div className="mt-4 pt-4 border-t border-black/10 flex flex-wrap items-end gap-3 font-mono text-xs">
          <div className="flex flex-col gap-1">
            <label htmlFor="custom-from" className="text-muted">
              from:
            </label>
            <input
              id="custom-from"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="custom-to" className="text-muted">
              to:
            </label>
            <input
              id="custom-to"
          ...
          <span className="text-muted ml-auto">// up to a 2-year span</span>
        </div>
      )}
    </ClipFrame>
  );
