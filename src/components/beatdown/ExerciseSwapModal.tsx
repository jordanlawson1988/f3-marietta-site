...
export default function ExerciseSwapModal({ focus, currentTerm, onApply, onClose }: Props) {
  ...
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/40 p-2">
      <div ref={dialogRef} className="w-full max-w-md rounded-lg bg-bone-2 border border-line-soft p-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <input
          ...
          className="w-full rounded-md border border-line-soft bg-bone-2 text-ink px-3 py-2 text-base mb-3"
        />
        <input
          value={reps}
          onChange={e => setReps(e.target.value)}
          placeholder="Reps (e.g., x 25 IC)"
          className="w-full rounded-md border border-line-soft bg-bone-2 text-ink px-3 py-2 text-sm mb-3"
        />
        <div className="flex-1 overflow-y-auto -mx-2">
          {results.map(r => (
            <button
              ...
              className="w-full text-left px-2 py-2 rounded hover:bg-bone-3"
            >
              <div className="font-medium">{r.term}</div>
              <div className="text-xs text-muted line-clamp-2">{r.shortDescription}</div>
            </button>
          ))}
          {results.length === 0 && <div className="text-sm text-muted px-2 py-4">No matches.</div>}
        </div>
      </div>
    </div>
  );
