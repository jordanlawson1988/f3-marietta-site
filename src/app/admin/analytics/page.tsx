...
import { MonthlyRecapPreview } from "@/components/stats/MonthlyRecapPreview";
...
export default async function AnalyticsOverviewPage({
  ...
  const aoHref = (slug: string) => `/admin/analytics/ao/${slug}${linkSuffix}`;
  const paxHref = (paxKey: string) => {
    const label = stats.topPax.find((p) => p.key === paxKey)?.label ?? paxKey;
    return `/admin/analytics/pax/${nameToSlug(label)}${linkSuffix}`;
  };
  const paxSlugHref = (slug: string) => `/admin/analytics/pax/${slug}${linkSuffix}`;
  const fngsHref = `/admin/analytics/fngs${linkSuffix}`;
  const qsHref = `/admin/analytics/qs${linkSuffix}`;
  ...
  const exportParams = `range=${range.slug}${
    range.slug === "custom"
    ...
    <section className="max-w-[1320px] mx-auto px-7 py-16">
      ...
      <div className="mt-8 pt-6 border-t border-line-soft">
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          <ExportButton
            href={`/admin/analytics/export?scope=overview&${exportParams}`}
            label="Overview CSV"
          />
          <ExportButton
            href={`/admin/analytics/export?scope=raw`}
            label="Raw attendance YTD"
          />
        </div>
      </div>
