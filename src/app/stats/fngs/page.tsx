...
export default async function StatsFngsPage({
  searchParams,
}: {
  ...
  if (!range) {
    if (sp.range || sp.from || sp.to) redirect("/stats/fngs");
  ...
  // Filter entries by AO chip selection in JS (getFngsList accepts no AO arg today).
  ...
  // Build AO option list from observed entries (the FNG list itself).
  const aoOptions = (() => {
  ...
  return (
    <section className="max-w-[1320px] mx-auto px-7 py-16">
      ...
      <StatsNav current="/stats/fngs" filterSuffix={linkSuffix} scope="customer" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <KpiTile
          ...
          caption="of FNGs called out, ever posted again"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
        <div className="md:col-span-7">
          <TrendChart
            ...
            subtitle="Counts each FNG only once, on the month of their first callout."
          />
        </div>
