...
export default async function StatsQsPage({
  searchParams,
}: {
  ...
  if (!range) {
    if (sp.range || sp.from || sp.to) redirect("/stats/qs");
  ...
  const [overview, qLeaderboard, notYetQd] = await Promise.all([
    ...
    getNotYetQdPax(range, aoFilter, 100),
  ]);
  ...
  const insight =
    totalQs === 0
      ...
      : `${totalQs} different PAX took the Q in ${range.label.toLowerCase()}, leading ${totalWorkoutsLed} workouts. ${topQName} led the rotation.`;
  ...
  return (
    <section className="max-w-[1320px] mx-auto px-7 py-16">
      ...
      <StatsNav current="/stats/qs" filterSuffix={linkSuffix} scope="customer" />
      ...
      <QLeaderboardTable
        ...
        paxHref={paxSlugHref}
        title="Q leaderboard"
        subtitle="Every PAX who Q'd at least one workout in this range. Drill into a PAX to see their full profile."
      />
