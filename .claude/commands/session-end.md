...
## Step 2: Check Test Health

Note whether tests were affected by this session's work:
- If code was modified, suggest running `/test-report` to verify nothing broke
- If tests were already failing before the session, note that

## Step 3: Update Context Files (if stale)

Check each living context file and update if the session's work changed their accuracy:

- **test-health.md** — If tests were run or test files were modified
- **feature-status.md** — If features were added, completed, or modified
- **architecture-notes.md** — If tech debt was added/resolved or dependencies changed
- **business-context.md** — Rarely changes; update only if business model or stakeholder info changed

## Step 4: Suggest Memory Updates

Review the session for anything worth persisting to memory (in `~/.claude/projects/` memory):
- New project context learned (deadlines, stakeholder feedback, scope changes)
...
==================================================
  ...
  Test State:
    E2E: [last known status]
    Delta: [any changes this session, or "not run"]
