...
# Adapted from Harman's Desserts hook for F3 Marietta project.
...
PROJECT_DIR="${CLAUDE_PROJECT_DIR:-/Users/jordan.lawson/Projects/f3-marietta}"
...
# Read the .env.local SUPABASE_URL (F3 uses SUPABASE_URL without NEXT_PUBLIC_ prefix)
...
ENV_URL=$(grep -E '^(NEXT_PUBLIC_)?SUPABASE_URL=' "$ENV_FILE" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '[:space:]"'\''')
